...
class School:
    ...
    def check_staff_id(self, id):
        return any(each.employee_id == id for each in self.staff)

    def check_password(self, id, pswd):
        return any(each.employee_id == id and each.password == pswd for each in self.staff)